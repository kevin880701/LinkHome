...
import 'package:haohsing_flutter/utils/datePicker/core/core.dart';
import 'package:haohsing_flutter/utils/datePicker/core/src/localizations/global_localizations.dart';
import 'package:haohsing_flutter/utils/datePicker/core/src/theme/daterangepicker_theme.dart';
import 'package:intl/intl.dart' show DateFormat;
import '../../../../../data/Params.dart';
import '../../../core/core_internal.dart';
...
class _PickerHeaderViewState extends State<_PickerHeaderView> {
  ...
  Widget build(BuildContext context) {
    ...
    if (showNavigationArrow) {
      arrowWidth = widget.width / 4;
      arrowWidth = arrowWidth > 160 ? 160 : arrowWidth;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int j = 0; j < count; j++) {
      ...
      for (int i = 0; i < DateTime.daysPerWeek; i++) {
        ...
        // 顯示中文星期
        String dayText = weekdayMap[DateFormat.EEEE().format(currentDate)]!;
...
class _PickerViewState extends State<_PickerView>
  ...
  @override
  void initState() {
    // 處理release模式時日程表首次開啟黑屏問題，強制刷新
    Future.delayed(Duration(milliseconds: 500)).then((_){
      final DateTime today = DateTime.now();
      _drawSelection(today);
      widget.updatePickerStateDetails(_pickerStateDetails);
    });

    _drawSelection(widget.visibleDates[19]);
    widget.updatePickerStateDetails(_pickerStateDetails);
    super.initState();
  }
...
String _getMonthHeaderText(
  ...
  if ((!isHijri && numberOfWeeksInView != 6) &&
    ...
    if (isHijri) {
      ...
      // 調整月份標題
      text =
          '${middleDate.year}-${middleDate.month.toString().padLeft(2, '0')}';
    }
